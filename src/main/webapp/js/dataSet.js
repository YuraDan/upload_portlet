(function () {
    var //--- Настройки ---
        ...
        getColumnTemplate = function(dataSet){
            var columnTemplate = {},
                data,
                attributeName;
            ...
            if (dataSet.length !== 0){
                ...
                for(attributeName in data){
                    columnTemplate[attributeName] = '';
                }
            }

            return columnTemplate;
        },
        addRow = function(iDataForm, columnTemplate){
            iDataForm.setData(columnTemplate);
        },
        removeRows = function(dataSet, selectedRowIndexes, iDataView){
            DevExpress.ui.dialog.confirm('Вы уверены, что хотите удалить ' +
                (selectedRowIndexes.length === 1 ? 'объект' : selectedRowIndexes.length + ' объекта(ов)') +'?',
                ...
                .done(function (dialogResult) {
                    ...
                    for(i = selectedRowIndexes.length; i--; ){
                        dataSet.splice(i, 1);
                    }
    ...
    angular.module('dataSet', ['dataView', 'dataForm'])
        .component('grDataSet', {
            transclude: false,
            templateUrl: 'liferay-upload-portlet/html/dataSet.html',
            bindings:{
                confirm:'<'
            },
            require:{
            ...
            controller:['$scope', function ($scope) {
                var columnTemplate = getColumnTemplate(dataSet),
                    selectedRowIndexes = [],
                    iDataView,
                    iDataForm;
                ...
                //--- Настройки кнопок ---
                extend(settings.addRow, {
                    onClick: function(){
                        addRow.call(this, iDataForm, columnTemplate);
                    }
                });
                extend(settings.removeRows, {
                    bindingOptions: {
                        disabled: '!isSelectedRows'
                    },
                    onClick: function(){
                        removeRows.call(this, dataSet, selectedRowIndexes, iDataView);
                    }
                });
                    ...
                    iDataView = interfaceConnector.importInterface('iDataView',
                        {
                            ...
                            onSelectRows: function(rowIndexes){
                                ...
                                $scope.isSelectedRows = selectedRowIndexes.length !== 0;
                            },
                            //--- Выбрана строка данных ---
                            onRowSelect: function(rowIndex){
                                formRowIndex = rowIndex;
                                iDataForm.setData(dataSet[rowIndex]);
                            },
                            //--- Выбран атрибут ---
                    ...
                    iDataForm = interfaceConnector.importInterface('iDataForm',
                        {
                            ...
                            onSave: function(data){
                                ...
                                //--- Редактирование ---
                                    ...
                                    extend(dataSet[formRowIndex], data);
                                }
                            }
                        }
                    );
